...
def RunGame():
    ...
    def drawMovingTile((row_s, col_s, row_f, col_f, number, flag), movement_i):
       ...
       if (row_f < row_s): #MovementTowardsUp
          ...
          step = distance / 10
          TileBackRect.center = [centers_coordinates[row_s][col_s][0], centers_coordinates[row_s][col_s][1] - step * movement_i]
          TileRect.center =  [centers_coordinates[row_s][col_s][0], centers_coordinates[row_s][col_s][1] - step * movement_i]
       ...
       if (col_f < col_s): #MovementTowardsLeft
          ...
          step = distance / 10
          TileBackRect.center = [centers_coordinates[row_s][col_s][0] - step * movement_i, centers_coordinates[row_s][col_s][1]]
          TileRect.center = [centers_coordinates[row_s][col_s][0] - step * movement_i, centers_coordinates[row_s][col_s][1]]
    ...
    def MergeMovements():#in the "movements" list is currently possible to have 2 movements for the same tile or other disordinate combinations
       ...
       for x in range(len(movements)):
          for y in range(x+1, len(movements)):
             if (movements[x][4] == movements[y][4]) and (movements[x][2] == movements[y][0]) and (movements[x][3] == movements[y][1]):
                movements[x] = (movements[x][0], movements[x][1], movements[y][2], movements[y][3], movements[y][4], movements[y][5])
                to_be_del.append(y)
             elif (movements[x][5] == 1) and (movements[x][4]*2 == movements[y][4]) and (movements[x][2] == movements[y][0]) and (movements[x][3] == movements[y][1]):
                movements[x] = (movements[x][0], movements[x][1], movements[y][2], movements[y][3], movements[x][4], movements[x][5])
                movements[y] = (movements[y][0], movements[y][1], movements[y][2], movements[y][3], movements[x][4], movements[x][5])
    ...
    def CreateRandomTiles(n):
       ...
       for i in range(n):
          ...
          while (game_state[empt_c['row']][empt_c['col']] != 0) or ((empt_c['row'],empt_c['col'],4) in showing_up_tiles) or ((empt_c['row'],empt_c['col'],2) in showing_up_tiles):
             empt_c={'row' : randint(0,3), 'col' : randint(0,3)}
...
#------case: DYNAMIC - ANIMATION PROGRESSING
          ...
          busy_spot_flag = [[0 for x in range(4)] for y in range(4)] #0 = will remain as is, 1 = is the destination of a movement, 2 = is the destination of a sum
          for x in range(len(movements)):
                if (movements[x][5] == 0):
          ...
          if (game_dynamics['i'] < 11):#PROPER MOVEMENT OF TILES
             ...
             for row in range(4):
                for col in range(4):
                   ...
                   elif (game_state[row][col] != 0) and (busy_spot_flag[row][col] == 2) and (game_state[row][col] == game_state_premove[row][col]*2):
          ...
          if (game_dynamics['i'] >= 11):#SHOWING UP + SUMMING UP OF TILES
             ...
             for i in range(len(showing_up_tiles)):
                showupTile(showing_up_tiles[i][0] , showing_up_tiles[i][1], showing_up_tiles[i][2] , game_dynamics['i']-11)
